...
const ADDR_TEST = "0xf95AFECc9bC8002bf22157715CCf50c0B99bF804";
...
async function testTX() {
  const functionSignatureHash = ethers.utils.id("pause()").slice(0, 10);
  ...
  const txPause = await signer.sendTransaction({
    to: ADDR,
...
const contractTest = new ethers.Contract(ADDR_TEST, ABIT, providerSepolia);
// Listen for Transfer events

async function test() {
  const functionSignatureHash = ethers.utils.id("unPause()").slice(0, 10);
  const supplySepolia = await contractSepolia.totalSupply();
  const supplyPolygon = await contractPolygon.totalSupply();

  if (supplySepolia.toNumber() < supplyPolygon.toNumber()) {
    console.log("yaaasss");
  } else {
    console.log("neinnn");
  }
...
const init = async () => {
  contractTest.once("mint1", (minter) => {
    console.log("Wee");
    test();
  });
};
